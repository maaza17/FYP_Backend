  ...
  userModel.findOne({ email: req.body.email }, (err, user) => {
    if (user) {
      return res.status(200).json({
        error: true,
        message:
          "This email is already registered with another account! Kindly use a different email address.",
      });
    } else {
            ...
            .then((user) => {
              ...
              transport.sendMail(mailOptions, (error, info) => {
                ...
                res.status(200).json({
                  ...
                  user: user,
                  message:
                    "User successfully registered! Check email for account verification!",
                });
              });
            })
            .catch((err) => {
  ...
  userModel.findOne({ email: email, is_deleted: false }).then((user) => {
    ...
    if (!user) {
      ...
      return res.status(200).json({
        error: false,
        message:
          "Account verification pending. Follow the link sent to your registered email address to verify account!",
      });
    }
    // Check password
